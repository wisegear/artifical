...
<nav aria-label="Main navigation"><a href="{{ route('home') }}">The journal</a><a href="{{ route('about') }}">About</a>@auth @can('admin')<a href="{{ route('admin.posts.index') }}">Admin</a>@endcan<form action="{{ route('logout') }}" method="post">@csrf<button class="nav-link">Log out</button></form>@else<a href="{{ route('login') }}">Log in</a><a class="button small" href="{{ route('register') }}">Stay curious <span>↗</span></a>@endauth</nav></header>
