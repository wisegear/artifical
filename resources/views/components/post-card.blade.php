...
<div class="card-meta">@foreach($post->tags ?? [] as $tag)<a href="{{ route('home',['tag'=>$tag]) }}#journal">{{ $tag }}</a>@endforeach<time datetime="{{ $post->post_date->toDateString() }}">{{ $post->post_date->format('d/m/Y') }}</time></div>
