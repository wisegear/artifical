...
@section('content')
<article class="article"><a class="text-link" href="{{ route('home') }}#journal">← Back to the journal</a><header class="article-header"><div class="eyebrow">@foreach($post->tags ?? [] as $tag)<a href="{{ route('home',['tag'=>$tag]) }}#journal">{{ $tag }}</a>@endforeach @if($post->is_subscriber) · MEMBERS ONLY @endif</div><h1>{{ $post->title }}</h1><p class="standfirst">{{ $post->seo_summary }}</p><div class="card-meta"><time datetime="{{ $post->post_date->toDateString() }}">{{ $post->post_date->format('d/m/Y') }}</time><span>Artificially Curious</span></div></header>
